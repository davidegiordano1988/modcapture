...
public class Application extends Thread{  
    ...
    public void turnOnRec() {
        this.flagcap=true;
    }
    
    public void turnOffRec() {
        this.flagcap=false;
    }
    
    public void turnOnLive() {
        this.flaglive=true;
    }
    
    public void turnOffLive() {
        this.flaglive=false;
    }
    ...
    public String getStato() {        
        if (flaglive) {
            if (flagcap) return "In cattura";
            else return "Live";
        }
        else return "In Pausa";
    }
        ...
        try {
            ...
            //while(!t.equals("EXIT")) {      
                ...
                while(!t.equals("STOP")) {   
                    if (flaglive) {
                      try {
                        if (flagcap) stato="In cattura";
                        else stato="Live";
                        Robot r = new Robot(gd); 
                        Calendar oggi = Calendar.getInstance();
                        //Dimension dim1=Toolkit.getDefaultToolkit().getScreenSize();
                       // larghezza=dim1.width;
                        //lunghezza=dim1.height;
                        //System.out.println(larghezza+"x"+lunghezza);
                        Dimension dim=new Dimension();
                        dim.width=Integer.parseInt(args[4]);
                        dim.height=Integer.parseInt(args[5]);
                        ...
                        String filestr=last+"/temp/"+oggi.get(Calendar.HOUR_OF_DAY)+"_"+oggi.get(Calendar.MINUTE)+"_"+oggi.get(Calendar.SECOND)+"_"+oggi.get(Calendar.MILLISECOND)+".jpg.tmp";
                        ...
                        Path path=Paths.get("D:\\Screenshots\\0\\LIVE\\live.jpg");
